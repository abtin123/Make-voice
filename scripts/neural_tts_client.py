...
class NeuralTtsClient:
    ...
    async def _generate(self, text: str, voice: str, speed: float) -> NeuralTtsResult:
        audio = bytearray()
        timestamps: list[dict[str, Any]] = []
    ...
    def synthesize(
        ...
        *,
        ...
        # `gender` is preserved for the old single-Persian CLI. Group builds
        # always pass an explicit language-specific Neural voice.
        selected_voice = voice or self._voice_by_gender.get(gender or "")
        if selected_voice is None:
            raise NeuralTtsError("Provide a supported Neural voice or Persian gender")
        last_error: Exception | None = None
        for attempt in range(1, self._attempts + 1):
            try:
                return asyncio.run(self._generate(text, selected_voice, speed))

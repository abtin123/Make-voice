...
class AvashoClient:
    ...
    def _bootstrap_session(self) -> None:
        ...
        # خود لینک دانلودی که در مستندات Avasho آمده، روی بعضی gatewayها
        # تنها مسیری است که کوکی نشست و CSRF متناظر را صادر می‌کند.
        for path in ("/download", "", "/", "/csrf", "/csrf-token"):
        ...
        for attempt in range(1, self._request_attempts + 1):
            ...
            try:
                ...
                if error.code in {401, 403}:
                    # نشست CSRF ممکن است بین submitهای بلندمدت منقضی شود؛
                    # پیش از retry آن را دوباره از gateway می‌گیریم.
                    self._csrf_token = ""

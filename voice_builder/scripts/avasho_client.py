...
class AvashoClient:
    ...
    def __init__(self, token: str, csrf_token: str = "", session_cookie: str = "",
        ...
        if not self._csrf_token:
            raise AvashoError(
                "Avasho did not return a CSRF token during session bootstrap. "
                "The gateway must expose a CSRF header, response field, or CSRF cookie."
            )
        if not self._session_cookie:
            raise AvashoError(
                "Avasho did not return a session cookie during session bootstrap. "
                "The gateway token is valid, but the gateway must create a temporary session."
            )
